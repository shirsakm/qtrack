...
async function cleanupActiveSessions() {
  ...
  try {
    ...
    const activeCount = await db.get('SELECT COUNT(*) as count FROM sessions WHERE is_active = 1');
    ...
    if (closedSessions.length > 0) {
      ...
      closedSessions.forEach((session, index) => {
        console.log(`   ${index + 1}. ${session.course_name} (${session.course_code}) - Section ${session.section}`);
